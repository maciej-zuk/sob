...
static Sint64 __zzip_seek(SDL_RWops *context, Sint64 offset, int whence)
{
    return zzip_seek(SDL_RWOPS_ZZIP_FILE(context), offset, whence);
}

static size_t __zzip_read(SDL_RWops *context, void *ptr, size_t size, size_t maxnum)
{
    return zzip_read(SDL_RWOPS_ZZIP_FILE(context), ptr, size*maxnum) / size;
}

static size_t __zzip_write(SDL_RWops *context, const void *ptr, size_t size, size_t num)
{
    return 0; /* ignored */
}
...
SDL_RWops *SDL_RWFromZZIP(ZZIP_FILE* zzip_file)
{
